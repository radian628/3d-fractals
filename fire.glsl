...
void main() {
	...
	if (shadowDistToSurface > uHitThreshold * 0.5 || sign(shadowRay.x - lambertLightLocation.x) != sign(dist.x - lambertLightLocation.x)) {
		colorValue = (1.0 - float(steps1) / 32.0);
		...
		colorValue = (1.0 - float(steps1) / 32.0) * uShadowBrightness;
	}
	...
	if (colorValue > 3.0) {
		...
		gl_FragColor = vec4(1.0, 1.0, colorValue - 2.0, 1.0);
	} else if (colorValue > 1.0) {
		gl_FragColor = vec4(1.0, colorValue - 1.0, 0.0, 1.0);
	} else {
